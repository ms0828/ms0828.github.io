...
HTTP는 클라이언트와 서버 간의 데이터 교환을 위한 Application Layer Protocol로, 메시지 형식과 요청/응답의 흐름을 정의하며 클라이언트-서버 모델을 기반으로 동작한다.<br>
...
2. **객체 전송**<br>
    하나의 TCP 연결에서 최대 한 개의 객체만 전송한다. 예를 들어, 클라이언트가 이미지가 포함된 HTML 파일을 요청하면 해당 TCP 연결에서 오직 HTML 파일 하나만 전송된다. 
3. **TCP 연결 닫기**<br>
    하나의 객체가 전송되고 나면, TCP 연결이 닫힌다. 이후 추가로 다른 객체(이미지)를 다운로드하려면 새로운 TCP 연결을 열어야 한다.
...
> 따라서 한 개의 객체당 Response time은 2RTT + 파일 전송 시간
...
- 사용자 데이터를 요청 메시지의 URL 필드에 포함하며, 일반적으로 URL 뒤에 '?' 문자를 붙여 쿼리 매개변수로 전달하기도 함
3. Head method
- HTTP GET 메서드로 요청한 URL에 대해 서버가 반환할 헤더만 요청할 때 사용
- 실제 콘텐츠는 요청하지 않고, 헤더 정보만 반환 (주로 디버그 목적)
4. Put method
...
웹 서버와 클라이언트(브라우저)는 트랜잭션 간의 상태를 유지하기 위해서 **쿠키(Cookies)** 를 사용 <br>
- 이는 HTTP 프로토콜이 자체적으로 상태를 유지하지 않는 것과는 다른 차원임, 쿠키는 서버의 데이터베이스에 사용자 정보와 함께 저장되어 특정 쿠키에 해당하는 사용자를 추적(맞춤형 서비스)하기 위해 사용
<br>
...
Web Cache는 사용자가 웹 사이트를 방문할 때 웹 페이지나 이미지 등 자주 호출되는 객체의 사본을 저장하여, 다음에 동일한 요청이 발생할 때 더 빠르게 응답할 수 있도록 돕는 또 다른 네트워크 개체이다. 
