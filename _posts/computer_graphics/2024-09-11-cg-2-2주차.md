...
VAO는 정점 배열 속성이 VBO에 저장된 데이터를 어떻게 해석하고 사용할지에 대한 연결을 저장하는 객체이다. 이를 통해 VAO는 VBO와의 연결 정보를 기억하고, GPU가 해당 데이터를 올바르게 처리할 수 있도록 한다.

> VAO는 VBO와 같은 여러 버퍼 객체의 상태와 속성을 저장한다. 즉, OpenGL에서 그리기 명령을 내리기 전에 정점 데이터를 어떻게 해석해야 하는지에 대한 설정을 VAO에 저장한다.

<br>

> 정점 속성은 뭔데?<br>
> - 메모리에 저장된 정점 데이터를 어떻게 해석할지를 정의하는 정보이다.
...
- glBufferData(~~): 지정된 VBO에 데이터를 실제로 업로드하는 함수로, GPU에 메모리가 복사되는 과정이 일어난다.
	 > * 파라미터 <br>
      1. 버퍼의 대상 (어떤 버퍼인지)<br>
